...
class request:
    ...
    def request_head(self, conn):
        ...
        # 请求字段的字典
        re_field = re.findall('(.*)\s?:\s?(.*)', data.decode())
        if re_field:
            self.re_field = {k.lower(): v for k, v in re_field}
    ...
    def _request_body(self, conn, data):
        ...
        length = re.search(r'Content-Length\s?:\s?(\d*)', data.decode())
...
class method:
    ...
    def post(self, conn, data, file_name):
        # 给响应体变量赋值
        if "content-type" not in self.re_field:
            # 如果没有这个字段，就无法识别他的请求体，所以返回空。正常的webserver也这样
            conn.sendall("")
            conn.close()
            return None
    ...
    def options(self, conn, data, file_name):
        # 实现的还不完整
        status, body = self.read(file_name)
        kw = {"Access-Control-Allow-Methods": ', '.join(self.__method__)}
        response = self.response_join(status, body, **kw)
        conn.sendall(response)
...
class response_head:
    ...
    def _response_head(self, length=None, **kw):
        '''请求头'''
    ...
    def response_join(self, status, body, **kw):
        '''拼接响应头额响应体'''
        length = self.clength(body)
        response = status + self._response_head(length, **kw) + body
        return response
...
class session(request, method, response_head, response_body):
    '''每个会话一个实例，这样多线程就不会出问题'''
...
# 0.9
# 请求头字段的字典， options方法，
# 分成两个类  一个是启动用的类，一个是每次会话的类，这样多进程就不会出现问题了

# 1.0
# 修改post请求，必须要有Content-Type头部字段，
# 没有Content-Type字段的请求无法读取请求体
